...
static t_ast	*parse_redir(t_data *minishell, t_token *tokens, t_token *op)
{
	if (!tokens || !op)
		return (NULL);
	if (tokens == op)
...
static t_ast	*parse_operators(t_data *minishell, t_token *tokens,
		t_token *op)
{
	t_ast	*node;
	t_token	*right;
...
static t_ast	*parse_token(t_token *tokens)
{
	t_ast	*node;
	int		count;
